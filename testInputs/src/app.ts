import {
    WorldBuilder
} from "./DesignPatterns/Strategy/WorldBuilder";
import {
    PerlinNoise
} from "./DesignPatterns/Strategy/PerlinNoise";
import {
    NoiseFactory
} from "./DesignPatterns/Factory/NoiseFactory";
import {
    BulletPool
} from "./DesignPatterns/ObjectPool/BulletPool";
import {
    ApartmentBuilder
} from "~DesignPatterns/Builder/Builder";
import {
    IDatabaseAdapter,
    HighscoreData,
    DatabaseAdapter
} from "./DesignPatterns/Adapter/Adapter";
import {
    ShapeDrawer,
    IShape
} from "./DesignPatterns/Facade/Facade";
import {
    Player,
    Achievements
} from "./DesignPatterns/Observer/Observer";
import {
    PlayerInput,
    Input
} from "./DesignPatterns/State/State";
...
let scoreboardBanner : PIXI.Sprite;
let scoreboardNameText : PIXI.Text;
let scoreboardScoreText : PIXI.Text;
...
app.loader.add('background', backgroundImg)
    ...
    .load((loader, resources) => {
        ...
        scoreboardBanner = new PIXI.Sprite.from(greenImg);
        scoreboardBanner.anchor.x = 0.5;
        scoreboardBanner.anchor.y = 0.5;
        scoreboardBanner.position.set(app.renderer.width / 2, app.renderer.height / 2);
        scoreboardBanner.width = app.renderer.width / 2;
        scoreboardBanner.height = app.renderer.height * 0.45;
        ...
        scoreboardNameText = new PIXI.Text("", style);
        scoreboardNameText.anchor.x = 0.5;
        scoreboardNameText.anchor.y = 0.5;
        scoreboardNameText.position.set(app.renderer.width * 0.4, app.renderer.height / 2);
        ...
        scoreboardScoreText = new PIXI.Text("", style);
        scoreboardScoreText.anchor.x = 0.5;
        scoreboardScoreText.anchor.y = 0.5;
        scoreboardScoreText.position.set(app.renderer.width *0.7, app.renderer.height / 2);
...
function MoveSnake(): void {
    ...
    switch (direction) {
        case Directions.UP:
            nextPos = new Vector2Int(snakePos[0].x, snakePos[0].y - 1);
            break;
        case Directions.RIGHT:
            nextPos = new Vector2Int(snakePos[0].x + 1, snakePos[0].y);
            break;
...
function GameOver(): void {
    ...
    console.log(UpdateDB(name, snakePos.length - 1));

    restartButton.position.set(app.renderer.width / 2, app.renderer.height * 0.85);
    app.stage.addChild(restartButton);
...
function Restart(): void {
    ...
    app.stage.removeChild(scoreboardBanner);
    app.stage.removeChild(scoreboardNameText);
    app.stage.removeChild(scoreboardScoreText);
...
function UpdateHighscoreboard() : string
{
    ...
    req.onload = function () {
        ...
        let array = raw.split("/");
        let regex = /\|/g;
        scoreboardNameText.text = array[0].replace(regex, "\n");
        scoreboardScoreText.text = array[1].replace(regex, "\n");
    };
    req.send();
}